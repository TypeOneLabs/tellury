...
// ascending by timestamp. "p95" is the client-side percentile the GCP
// aggregation contract requires (REDUCE_NONE, then nearest-rank percentile
// over the window); the rest are plain reductions over the aligned window.
//
...
func reduceTimeStat(spec Spec, series metrics.Series) (float64, error) {
	...
	default:
		return 0, fmt.Errorf("gcp metrics: unsupported time_stat %q", spec.TimeStat)
	}
}
